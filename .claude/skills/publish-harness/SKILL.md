...
반환된 `code` 필드로 분기:
- `BOOTSTRAP_REQUIRED` / `FIGMA_TOKEN_MISSING` / `TOKENS_MISSING` → 사용자에게 해당 명령 안내 (`recommendations` 필드 그대로 노출)
...
추가 컨텍스트가 필요할 때만 `bash scripts/status.mjs --json` 으로 `phase` / `recommendedPhase` / `nextActionable` 을 조회한다. PROGRESS.md / progress.json 을 직접 파싱하지 않는다 — 모든 추론은 위 세 CLI 가 수행한다.
...
`docs/project-context.md` 의 `mode` 필드에 따라 분기한다. 공통 원칙: **워커 스폰 없이 오케스트레이터가 직접 수행**, 결과는 `progress-update` CLI 로 `progress.json` 에 반영 (PROGRESS.md 는 자동 렌더), 마지막에 사용자 승인 1회. 오케스트레이터는 `PROGRESS.md` 를 직접 편집하지 않는다.
...
7. `progress-update` CLI 로 페이지 + 섹션 목록 등록 — "공통 컴포넌트 먼저" 규칙에 따라
   DS 인벤토리에서 식별한 컴포넌트를 Phase 3 맨 앞에 스폰. `PROGRESS.md` 직접 편집 금지 (자동 렌더).
   ```bash
   bash scripts/progress-update.mjs add-page --name <page> --node-id <node-id>
   bash scripts/progress-update.mjs add-section --name <section> --page <page> --kind section
   # 공통 컴포넌트는 page 없이 등록
   bash scripts/progress-update.mjs add-section --name <Component> --kind component
   ```
...
6. `progress-update` CLI 로 컴포넌트 카탈로그 등록 — 섹션 워커 스폰 순서대로. `PROGRESS.md` 직접 편집 금지.
   ```bash
   bash scripts/progress-update.mjs add-section --name <Component> --kind component
   ```
...
2. git 커밋 (자동):
   ...
   ```
3. 게이트 결과 반영: **A.7 부터** `measure-quality.sh` 가 `progress-update record-gate-result` 를 자동 호출하여 PASS 시 섹션을 done 으로 마크 + `PROGRESS.md` 자동 렌더할 예정. 오케스트레이터가 PROGRESS.md 를 직접 수정하지 않는다. 그 전까지는 수동 실행:
   ```bash
   # result-file 은 measure-quality.sh 가 자동 생성하는 JSON. 사용자가 직접 만들 일은 거의 없음.
   bash scripts/progress-update.mjs record-gate-result --section <section> --result-file tests/quality/<section>.json
   ```
4. 다음 단위로 즉시 진행
...
3. **retry_count=2 FAIL (needs_human: true) → 사용자 개입**
   - 누적 failures 요약 + 선택지 제시:
     ...
     - (b) 수동 리팩터 (사용자가 직접)
     - (c) 섹션 스킵 (다음으로 넘어감, `bash scripts/progress-update.mjs set-section --name <section> --status skipped` — 사유는 commit message 또는 별도 문맥에 남길 것)
     - (d) 섹션 재분할 (서브섹션으로 쪼개기)

**안티-루프 가드**: 각 재스폰 전 `bash scripts/why.mjs --json` 호출 — `code: ANTI_LOOP_TRIGGERED` 반환 시 자동 재스폰 중단하고 사용자 개입 (`recommendations` 그대로 노출). opus 승격 / 재분할 등 분기는 why.mjs 의 `recommendations` 가 결정한다.
...
- 차단 동기화: `bash scripts/progress-update.mjs set-section --name home-footer --status blocked` (사유 "blocked by home-header" 는 commit message 또는 별도 문맥에 기록 — PROGRESS.md 자동 렌더)
...
1. `progress.json` (또는 PROGRESS.md 자동 렌더 결과) 에서 해당 페이지의 모든 섹션이 done 인지 확인 — 직접 PROGRESS.md 를 편집하지 말 것
...
6. 페이지 단위 done 은 별도로 마크업하지 않는다 — 섹션이 모두 done 이면 `progress.json` 상으로 페이지 완료 상태가 자동 표현됨
...
| 오케 → 사용자 | `progress-update` 호출 결과 (PROGRESS.md 자동 렌더) + 간단 통보 |
...
**정상 흐름**: 사용자 "다음 섹션 진행"
1. `bash scripts/next.mjs --json` → `target` 으로 다음 미완 섹션 식별
2. docs/project-context.md에서 nodeId 조회
3. section-worker 스폰
4. 반환 JSON 검증 → PASS → 자동 커밋 → `progress-update record-gate-result` 자동 반영 (PROGRESS.md 자동 렌더) → 다음 섹션 제안
