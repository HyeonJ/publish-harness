...
7. `progress-update` CLI 로 페이지 + 섹션 목록 등록 — "공통 컴포넌트 먼저" 규칙에 따라
   ...
   ```bash
   bash scripts/progress-update.mjs add-page --slug <page>
   bash scripts/progress-update.mjs add-section --page <page> --slug <section> --kind section
   # 공통 컴포넌트는 page 없이 등록
   bash scripts/progress-update.mjs add-section --slug <Component> --kind component
   ```
...
2. git 커밋 (자동):
   ...
   ```
3. 게이트 결과 반영: `measure-quality.sh` 가 `progress-update record-gate-result` 자동 호출하여 PASS 시 섹션을 done 으로 마크 + `PROGRESS.md` 자동 렌더. 오케스트레이터가 PROGRESS.md 를 직접 수정하지 않는다.
   ```bash
   # measure-quality 가 자동 호출. 수동 실행이 필요하면:
   bash scripts/progress-update.mjs record-gate-result --slug <section> --gate G1 --result PASS
   ```
4. 다음 단위로 즉시 진행
...
3. **retry_count=2 FAIL (needs_human: true) → 사용자 개입**
   - 누적 failures 요약 + 선택지 제시:
     ...
     - (b) 수동 리팩터 (사용자가 직접)
     - (c) 섹션 스킵 (다음으로 넘어감, `bash scripts/progress-update.mjs set-section --slug <section> --status skipped --notes "<사유>"`)
     - (d) 섹션 재분할 (서브섹션으로 쪼개기)
...
- 차단 동기화: `bash scripts/progress-update.mjs set-section --slug home-footer --status blocked --notes "blocked by home-header"` (PROGRESS.md 자동 렌더)
