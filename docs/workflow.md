...
오케스트레이터가 직접 수행:
0. Figma URL 에 `node-id` 가 없거나 페이지 테이블이 비어 있으면 route page discovery 먼저 수행:
   ```bash
   node scripts/discover-figma-pages.mjs --file-key <fileKey> --out docs/figma-pages.md --apply
   node scripts/progress-render.mjs
   ```
   Figma top-level `SECTION`/`FRAME` 중 `Home`, `/about`, `/find-us` 처럼 라우트 이름을 가진
   노드는 각각 별도 페이지로 취급한다. 대표 Desktop frame 하나만 골라 전체 프로젝트로
   해석하지 않는다.
1. 페이지 Node ID 확인 (`docs/project-context.md` 또는 `docs/figma-pages.md`)
