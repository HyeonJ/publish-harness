...
**Spec:** `docs/superpowers/specs/2026-04-28-strict-visual-harness-design.md` (v4, commit 4235ba0)

**v1 → v2 (이 문서)**: spec v4 의 §12.5 (figma `use_figma` 로 multi-viewport 자동 생성) 반영 — Task 22 신설, Task 15 의 workflow.md 갱신 sub-step 보강 (Phase 2 분기 처리 표).
...
- **`docs/responsive-figma-generator.md`** (v2 신규) — 오케스트레이터용 figma `use_figma` plugin API 코드 가이드 (mobile/tablet frame 자동 생성 패턴)
...
- `docs/workflow.md` — Phase 3.4.1/4.2 갱신, §baseline 갱신 프로토콜 추가, **Phase 2 분해 절의 반응형 프레임 감지에 4분기 자동 생성 흐름 추가 (v2)**
...
## Phase 6 — 가이드 문서 (T14~T16, T22 v2 신규)
...
- [ ] **Step 5: Phase 2 분해 절의 반응형 프레임 감지 갱신 (v2 신규)**

기존 workflow.md 의 Phase 2a §"반응형 프레임 감지" 절 (line 84 부근) 끝에 다음 추가:
...
```bash
grep -c "G11\|escape budget\|prepare-baseline\|use_figma\|4분기" docs/workflow.md
```
Expected: ≥ 7.
...
git commit -m "docs(strict): workflow — Phase 4.1/4.2 + baseline 갱신 + G11 + Phase 2 4분기 자동 처리"
```
...
### Task 22 (v2 신규): docs/responsive-figma-generator.md — use_figma plugin API 가이드

**Files:**
- Create: `docs/responsive-figma-generator.md`
- Modify: `scripts/bootstrap.sh` (docs/ 복사 항목 추가, 이미 docs/ 통째 복사라면 자동)

**Context**: 오케스트레이터가 Phase 2 분해 단계에서 figma `use_figma` MCP 도구로 mobile/tablet frame 자동 생성할 때, 일관된 plugin API 코드 패턴을 사용하도록 가이드. 워커가 아닌 오케가 페이지 단위로 호출.
...
- [ ] **Step 2: bootstrap.sh 의 docs/ 복사 확인**

```bash
grep -E "docs/.*\.md|cp.*docs" scripts/bootstrap.sh | head -10
```

기존에 `docs/` 통째 복사하면 자동 포함. 아니면 `cp "$HARNESS_DIR/docs/responsive-figma-generator.md" docs/` 추가.

- [ ] **Step 3: smoke**

```bash
test -f docs/responsive-figma-generator.md && grep -c "use_figma\|VIEWPORT_LABEL\|figma.createFrame" docs/responsive-figma-generator.md
```
Expected: ≥ 5.
...
```bash
git add docs/responsive-figma-generator.md scripts/bootstrap.sh
git commit -m "feat(strict): responsive-figma-generator 가이드 — use_figma plugin API 코드 패턴"
```
